...
static void raw_i2c_sw_reset(struct ln8000_info *info)
{
        u8 reg;

        i2c_smbus_write_byte_data(info->client, LN8000_REG_LION_CTRL, 0xC6);

        /* SOFT_RESET_REQ = 1 */
        reg = i2c_smbus_read_byte_data(info->client, LN8000_REG_BC_OP_2);
        reg = reg | (0x1 << 0);
        i2c_smbus_write_byte_data(info->client, LN8000_REG_BC_OP_2, reg);

        msleep(5 * 2);  /* ln8000 min wait time 5ms (after POR) */
}

static int try_to_find_i2c_address(struct ln8000_info *info)
{
        u8 addr_set[] = {0x55, 0x5f, 0x51, 0x5b};
        u8 ori_addr = info->client->addr;
        int i, ret;

        for (i = 0; i < ARRAY_SIZE(addr_set); ++i) {
                info->client->addr = addr_set[i];
                ret = i2c_smbus_read_byte_data(info->client, LN8000_REG_DEVICE_ID);
                if (ret == 0x42) {
                        ln_err("find to can be access address(0x%x)(ori=0x%x)\n",
                                 info->client->addr, ori_addr);
                        raw_i2c_sw_reset(info);
                        break;
                } else {
                        ln_err("can't access address(0x%x)(ori=0x%x)\n",
                                 info->client->addr, ori_addr);
                }
        }
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
		...
		msleep(250);
		ret = try_to_find_i2c_address(info);
		if (ret != 0x42) {
			ret = -EPROBE_DEFER;
